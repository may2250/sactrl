...
{
	LPSERVENT	lpservent;
	//LPCTSTR		pstrDevice;		// wangzheng 2007-4-2
	...
	if ( _istdigit( strServiceName[0] ) ) {
		...
		if ( (lpservent = getservbyname( pstrService/*pstrDevice*/, NULL )) != NULL )
...
///////////////////////////////////////////////////////////////////////////////
// DESCRIPTION:
//		Returns all IP address.
//			- It tries to convert the string directly
//			- If that fails, it tries to resolve it as a hostname
// PARAMETERS:
//	LPCTSTR strHostName: host name to get IP address
//  ULONG* paddrList: Address list
///////////////////////////////////////////////////////////////////////////////
void CSocketComm::GetAllIPAddress( LPCTSTR strHostName, ULONG* paddrList, UINT nListLen )
{
	LPHOSTENT	lphostent;
	ULONG		uAddr = INADDR_NONE;
	TCHAR       strLocal[HOSTNAME_SIZE] = { 0 };
	...
	if ( (INADDR_NONE == uAddr) && (strcmp( strHost, "255.255.255.255" )) && (paddrList != NULL) )
	{
		// It's not an address, then try to resolve it as a hostname
		lphostent = gethostbyname( strHost );
		for ( UINT i=0; i<nListLen; i++ )
		{
			if ( lphostent->h_addr_list[i] == NULL  )
			{
				break;
			}
			uAddr = *((ULONG *) lphostent->h_addr_list[i]);
			paddrList[i] = ntohl( uAddr );
...
///////////////////////////////////////////////////////////////////////////////
SOCKET CSocketComm::WaitForConnection(SOCKET sock, sockaddr_in& clientaddr)
{
	...
	return accept(sock, (struct sockaddr *)&clientaddr, &iAddrSize);
}
...
bool CSocketComm::GetSockName(SockAddrIn& saddr_in)
{
	if (IsOpen())
	{
		int namelen = static_cast<int>(saddr_in.Size());
...
bool CSocketComm::CreateSocket(LPCTSTR strServiceName, int nFamily, int nType, UINT uOptions /* = 0 */)
{
	...
	if (INVALID_SOCKET != sock)
	{
		...
		SockAddrIn sockAddr;
		if(IsServer())
			sockAddr.CreateFrom(_T("0.0.0.0"), strServiceName, nFamily);
		else
			sockAddr.CreateFrom(NULL, strServiceName, nFamily);

		if ( SOCKET_ERROR == bind(sock, (LPSOCKADDR)sockAddr,  static_cast<int>(sockAddr.Size())))
		{
			closesocket( sock );
			m_bBroadcast = false;
			if (NULL != m_hMutex)
...
bool CSocketComm::ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, int nFamily, int nType, 
							bool no_blocking, long tv_sec, long tv_usec, LPCTSTR strBindAddr )
{
	// Socket is already opened
	if ( IsOpen() )
		return false;
	...
	if (INVALID_SOCKET != sock)
	{
		...
		if (false == sockAddr.CreateFrom(strBindAddr, TEXT("0"), nFamily))
		{
			closesocket( sock );
			return false;
		}
		...
		if( !no_blocking )		// 阻塞方式
		{
			...
			if (SOCKET_ERROR == connect( sock, (LPSOCKADDR)sockAddr, static_cast<int>(sockAddr.Size())))
			{
				closesocket( sock );
				return false;
			}
		...
		{
			unsigned long lno_blocking = 1;
			int ret = ioctlsocket(sock,FIONBIO,&lno_blocking);
			if(ret==SOCKET_ERROR)
				return false;

			connect( sock, (LPSOCKADDR)sockAddr, static_cast<int>(sockAddr.Size()));
			//select 模型，即设置超时
...
//	_ASSERTE( IsOpen() );
	...
	if ( INFINITE != dwTimeout ) {
		...
		stTime.tv_usec = (dwTimeout - stTime.tv_sec*1000)*1000;
		pstTime = &stTime;
	}
	...
	int res = select( static_cast<int>(s+1), &fdRead, NULL, NULL, pstTime );
	if ( res > 0)
	{
		if (IsBroadcast() || IsSmartAddressing())
		{
			SockAddrIn sockAddr;
			int nLen = static_cast<int>(sockAddr.Size());
			...
			if ( res >= 0)
			{
				...
				if (IsSmartAddressing())
				{
					...
					res += static_cast<int>(sockAddr.Size());
				}
			}
...
DWORD CSocketComm::WriteComm(const LPBYTE lpBuffer, DWORD dwCount, DWORD dwTimeout)
{
	...
	int res = select( static_cast<int>(s+1), NULL, &fdWrite, NULL, pstTime );
	if ( res > 0)
	{
		...
		if (IsBroadcast() || IsSmartAddressing())
		{
			...
			if (IsSmartAddressing())
			{
				...
				if (sockAddr.sin_addr.s_addr != htonl(INADDR_BROADCAST))
				{
					...
					res = sendto( s, lpszData, dwCount-nOffset, 0,
						(LPSOCKADDR)sockAddr, static_cast<int>(sockAddr.Size()));
			...
			for( ; iter != m_AddrList.end(); )
			{
				...
				res = sendto( s, (LPCSTR)&lpBuffer[nOffset], dwCount-nOffset, 0, (LPSOCKADDR)(*iter), static_cast<int>(iter->Size()));
				if (res < 0)
				{
...
void CSocketComm::Run()
{
	...
	if (IsServer())
	{
		if (!IsBroadcast())
		{
			...
			sock = WaitForConnection( sock, clientaddr );

			strcpy(m_clientIP,inet_ntoa(clientaddr.sin_addr));   
...
char* CSocketComm::GetClientAddr()
{
	return m_clientIP;
}
